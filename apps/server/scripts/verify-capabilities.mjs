...
const root = `apps/server/src/main/kotlin/${group.replaceAll(".", "/")}`;
...
function verifyCapability(capability, root, groupName) {
  ...
  assert.ok(
    existsSync(`${root}/${capability.package}/api/${type}.kt`),
    `Missing Capability entry Interface for ${capability.id}`,
  );
  assert.ok(
    existsSync(`apps/server/src/main/resources/db/capability/${capability.schema}`),
  ...
  for (const useCase of capability.useCases ?? []) {
    ...
    assert.ok(
      existsSync(
        `${root}/${capability.package}/api/${useCase.requestType}.kt`,
      ),
      `Missing request contract for ${useCase.id}`,
    );
    assert.ok(
      existsSync(
        `${root}/${capability.package}/api/${useCase.outcomeType}.kt`,
      ),
